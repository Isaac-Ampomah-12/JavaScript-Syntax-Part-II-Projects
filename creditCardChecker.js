...
const findInvalidCards = (nestedArray) =>{
   ...
   for (let index = 0; index<nestedArray.length; index++){
       const validity = validateCred(nestedArray[index]);
       validity? validCards.push(nestedArray[index]) : invalidCards.push(nestedArray[index]);
   }
   return invalidCards;    
}
...
const idInvalidCardCompanies = (nestedArray) =>{
    ...
    for (let index = 0; index < invalidCards.length; index++){
        let determinant = invalidCards[index][0];
        switch (determinant){
            case 3: companies = 'Amex(American Express)';
                if(!companiesArray.find(element => element === companies)){
